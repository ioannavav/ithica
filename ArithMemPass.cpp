...
namespace {
	struct ArithMemPass : public FunctionPass {
		static char ID;
		ArithMemPass() : FunctionPass(ID) {}
		...
		bool isExcludedInstruction(Instruction *inst) {
			...
			// Conditional inclusion of Loads, Stores and function calls.
			...
			if (auto *callInst = dyn_cast<CallInst>(inst)) {
				...
				if (cast<CallInst>(inst)->getCalledFunction() && cast<CallInst>(inst)->getCalledFunction()->getName().contains("bzhi")) {
					return false;
				}
				if (cast<CallInst>(inst)->getCalledFunction() && cast<CallInst>(inst)->getCalledFunction()->getName().contains("movsb")) {
					return false;
				}
				if (cast<CallInst>(inst)->getCalledFunction() && cast<CallInst>(inst)->getCalledFunction()->getName().contains("stosb")) {
					return false;
				}
				...
				if (auto *callInst = dyn_cast<CallInst>(inst)) {
					if (Function *calledFunction = callInst->getCalledFunction()) {
						if (calledFunction->getName().contains("llvm.") && !callInst->getType()->isVoidTy()) {
							// Check if the called function has the `readonly` or `readnone` attribute
		...
		std::string instructionToString(Instruction *I) {
			...
			for (char c : rso.str()) {
				if (c == '%') {
					safeStr += "%_"; // Remove '%' to avoid wrong string printing
				} else {
					safeStr += c;
				}
			}
			...
			for (auto BI : bbs) {
					...
					|| (blockSize !=0 && (instCounter % blockSize == 0 || instCounter == originalInstructions.size()))) {
                        ...
                        if (lastValue) {
                            ...
                            // Pointers
                                ...
                                errs() << "No comparison supported for instruction: " << *Inst << " is of type: ";
                                lastInst->getType()->print(errs());
                                errs() << "\n";
                            }
                        }
				...
				if (!icmpResults.empty()) {
					...
					for (auto &icmpPair : icmpResults) {
						...
						//Since the allocation of space for the printing is happening at compile time, it could be the case that this changes at runtime after the optimizations are performed. For this reason, we allocate a big enough constant amount of space (64 bytes) for all instructions
						Value *SizeVal = elseBuilder.getInt32(64); // 64 bytes
						MaybeAlign Alignment(8); // Assume alignment of 1
					...
					//Remove the following logging for quicker building
	...
	unsigned ArithMemPass::blockCounter = 1;
	unsigned ArithMemPass::totalBasicBlocks = 0;
	bool ArithMemPass::countedBlocks = false;

	char ArithMemPass::ID = 0;


	static RegisterPass<ArithMemPass> X("arithmempass", "Insert arithmetic and memory cloned instructions with check", false, false);

	static void registerArithMemPass(const PassManagerBuilder & Builder, legacy::PassManagerBase &PM) {
		PM.add(new ArithMemPass());
	}

	static RegisterStandardPasses RegisterArithMemPass(PassManagerBuilder::EP_OptimizerLast, registerArithMemPass);
